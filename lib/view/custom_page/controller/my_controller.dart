...
class MyController {
  ...
  final listener = ValueNotifier<(double heightValue, double currentThreshHold)>((0, 0));
  ...
  void onHorizontalDragUpdate(DragUpdateDetails details) {
    final swipe = _startTapPosition.ceilToDouble() -
        details.localPosition.dy.ceilToDouble();
    ...
    if (isScrollUp) {
      ...
      if (holderThreshToCheck > 0 && holderThreshToCheck < totalPage - 1) {
        if (approxEqual(gestureHeight - 50, _holdPositionValue, 0.1) &&
            _needEndCall) {
  ...
  double calculateThreshHold(double value) {
    return _currentPage -
        normalize(value: value, oldMin: 0, oldMax: gestureHeight - 50);
  }
